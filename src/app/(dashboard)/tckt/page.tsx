    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-xl p-4 shadow-sm dark:shadow-none transition-all">
                    <p className="text-3xl font-bold text-slate-900 dark:text-white">{contracts.length}</p>
                    <p className="text-sm text-slate-600 dark:text-slate-400">Chờ thanh toán</p>
                </div>
                <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-xl p-4 shadow-sm dark:shadow-none transition-all">
                    <p className="text-3xl font-bold text-slate-900 dark:text-white">
                        {contracts.filter((c) => c.nguoiThanhToanId).length}
                    </p>
                    <p className="text-sm text-slate-600 dark:text-slate-400">Đã giao việc</p>
                </div>
                <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-xl p-4 shadow-sm dark:shadow-none transition-all">
                    <p className="text-3xl font-bold text-slate-900 dark:text-white">
                        {contracts.filter((c) => !c.nguoiThanhToanId).length}
                    </p>
                    <p className="text-sm text-slate-600 dark:text-slate-400">Chưa giao việc</p>
                </div>
            </div>
